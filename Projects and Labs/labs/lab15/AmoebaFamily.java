...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba> {
    ...
    public void printHelper(String lines, Amoeba r) {
        if (r != null) {
            System.out.println(lines + r.toString());
            for (Amoeba a : r.getChildren()) {
                printHelper(lines + "   ", a);
            }
        }
    }
    ...
    public String longestName() {
        if (root != null) {
            return longestNameHelper(longestNameLength(), root);
        }
        return "";
    }

    public String longestNameHelper(int longestNamelen, Amoeba a) {
        if (a.toString().length() == longestNamelen) {
            return a.toString();
        } else {
            String toReturn = null;
            for (Amoeba child: a.getChildren()) {
                toReturn = longestNameHelper(longestNamelen, child);
            }
            return toReturn;
        }
    }
    ...
    public class AmoebaDFSIterator implements Iterator<Amoeba> {
        ...
        public Amoeba next() {
            ...
            if (parent.getChildren() != null) {
                ...
                while (i >= 0) {
                    fringe.push(parent.getChildren().get(i));
                    i -= 1;
                }
            }
            return parent;
