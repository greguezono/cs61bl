...
public class Graph {
    ...
    public Graph prims(int start) {
        ...
        PriorityQueue<Integer> fringe = new PriorityQueue<>(1, (o1, o2) ->
                distFromTree.get(o1).getWeight() - distFromTree.get(o2).getWeight());
        TreeSet<Edge> neighborEdges = getEdges(start);
        for (Edge e: neighborEdges) {
            distFromTree.put(e.getDest(), e);
            fringe.add(e.getDest());
        }
        while (mst.getAllVertices().size() != getAllVertices().size()
                && mst.getAllEdges().size() != getAllEdges().size() - 1) {
            int v = fringe.poll();
            mst.addVertex(v);
            Edge e = distFromTree.get(v);
            mst.addEdge(e);
            neighborEdges = getEdges(v);
            for (Edge ed: neighborEdges) {
                int vToAdd = ed.getDest();
                if (!mst.containsVertex(vToAdd)) {
                    Edge distFromTreeEdge = distFromTree.get(vToAdd);
                    if (distFromTreeEdge == null || distFromTreeEdge.getWeight() > ed.getWeight()) {
                        distFromTree.remove(ed.getDest());
                        distFromTree.put(ed.getDest(), ed);
                        fringe.remove(ed.getDest());
                        fringe.add(ed.getDest());
                    }
                }
            }
    ...
    public Graph kruskals() {
        ...
        WeightedQuickUnionUF disjointSet = new WeightedQuickUnionUF(getAllVertices().size());
        PriorityQueue<Edge> fringe = new PriorityQueue<>(1,
            (o1, o2) -> o1.getWeight() - o2.getWeight());
        for (Edge e: getAllEdges()) {
            fringe.add(e);
        }
        while (!fringe.isEmpty() && mst.getAllEdges().size() != mst.getAllVertices().size() - 1) {
            Edge e = fringe.poll();
            if (!disjointSet.connected(e.getSource(), e.getDest())) {
                mst.addEdge(e);
                disjointSet.union(e.getSource(), e.getDest());
            }
        }
        return mst;
    }
