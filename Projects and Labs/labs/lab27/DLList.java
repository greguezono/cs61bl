...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        ...
        while (pointer.next != sentinel && pointer.next.item.compareTo(item) < 0) {
            pointer = pointer.next;
        }
        Node toInsert = new Node(item, pointer.prev, pointer.next);
        pointer.prev.next = toInsert;
        pointer.next.prev = toInsert;
    }
    ...
    public DLList<T> mergeSort() {
        ...
        DLList<T> sortedHalf = oneHalf.mergeSort();
        DLList<T> sortedOtherHalf = otherHalf.mergeSort();
        return sortedHalf.merge(sortedOtherHalf);
    }
    ...
    public DLList<T> quicksort() {
        ...
        while (pointer != sentinel) {
            ...
            if (item.compareTo(pivot) == 0) {
                equalElements.addFirst(item);
            } else if (pivot.compareTo(item) > 0) {
                smallElements.addFirst(pointer.item);
            } else {
                largeElements.addFirst(pointer.item);
            }
            pointer = pointer.next;
        }
